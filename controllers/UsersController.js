...
exports.GetUserProfile = async (req, res) => {
  try {
    ...
    const { password, rememberMe, loginAttempts, lockUntil, ...userData } =
      user._doc;
...
exports.UpdateProfile = async (req, res) => {
  ...
  try {
    ...
    const updatedProfile = await User.findByIdAndUpdate(_id, updatedInfo, {
      new: true,
    });

    res
      .status(200)
      .json({ message: "Account updated successfully", user: updatedProfile });
  } catch (error) {
    console.log(error);
    res.status(500).json({ message: "Internal server error" });
  }
...
exports.ResetPassword = async (req, res) => {
  ...
  if (passwordSchema.error) {
    return res
      .status(400)
      .json({ message: passwordSchema.error.details[0].message });
  }
...
exports.DeleteAccount = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Account deleted successfully", redirect: "/" });
  } catch (error) {
    res.status(500).json({ message: "Internal server error" });
  }
};
